...
AUTH0_DOMAIN = 'coffeeudacity.auth0.com'
ALGORITHMS = ['RS256']
API_AUDIENCE = 'coffee'
...
def get_token_auth_header():
    ...
    if not auth:
        raise AuthError('Non authorization header', 401)

    parts = auth.split()
    if parts[0].lower() != 'bearer':
        raise AuthError('Authorization header not Bearer', 401)

    elif len(parts) == 1:
        raise AuthError('No Token', 401)
...
def check_permissions(permission, payload):
    ...
    if permission not in payload['permissions']:
        raise AuthError('No Permissions', 403)
...
def verify_decode_jwt(token):
    ...
    if rsa_key:
        try:
            ...
            raise AuthError('Bad Claim', 401)

        # In all other Error cases, give generic error message
        except Exception:
            raise AuthError('Bad Header', 400)
